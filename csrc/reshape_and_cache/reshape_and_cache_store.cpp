...
void kv_cache_flat_to_blocked_hvx(
  ...
  while (tok < (uint32_t)seqlen) {
    ...
    while (hid < (uint32_t)nheads_k) {
      while (inheaddim < headdim_size) {
        src_value_offset =
            inheaddim + src_token_stride * tok + src_kv_head_stride * hid;
        dest_value_offset = block_stride * block_id + dst_head_stride * hid +
                            block_offset * src_kv_head_stride + inheaddim;

        v_base[dest_value_offset] = value[src_value_offset];
        src_key_offset =
            inheaddim + src_token_stride * tok + src_kv_head_stride * hid;
        dest_key_offset = block_stride * block_id + dst_head_stride * hid +
                          block_offset * src_kv_head_stride + inheaddim;
        k_base[dest_key_offset] = key[src_key_offset];
...
QAIC_KERNEL_API int32_t multinsp_multithread_kv_cache_flat_to_blocked_hvx_f(
  ...
  kv_cache_flat_to_blocked_hvx(key, value, key_cache, value_cache, buff, buffer,
