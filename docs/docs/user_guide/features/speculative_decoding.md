...
!!! tip "Quick recommendation"
    Start with **ngram** SpD — it's zero-cost (no extra model, no core reallocation) and works well for summarization and conversational workloads. Switch to **draft_model** if you need higher acceptance rates on diverse generation tasks.

## Methods

Cloud AI supports three speculative decoding methods:

| Method | Proposer | Extra Model | Extra Cores | Best For |
|--------|----------|-------------|-------------|----------|
...
!!! note "Triton-CPU backend required for AOT SpD"
    For AOT mode rejection sampler uses Triton kernels running on CPU,
...
Zero-cost method that requires no additional model or core allocation:
...
## Core Allocation (Need more clarrification)
...
??? tip "Tuning parameters"
    ...
    | `num_cores` (target) | Compute for verification | 10-12 for 8B models |
    | `num_cores` (draft) | Compute for proposing | 4-6 for 1B models |
