...
class QAicAttentionBackendImpl(AttentionImpl):
    ...
    def forward(
        ...
        if num_actual_tokens > 0:
            self.attention_with_kv_cache(
                ...
                s_aux=self.sinks if self.sinks is not None else None,
    ...
    def write_to_paged_cache(
        ...
        # slice and assign kv instead of transpose
    ...
    def attention_with_kv_cache(
        ...
        # read kv cache and create a single linear tensor
        block_size = key_cache.size(1)

        # FIXME check alibi bias and sliding window bias in decode inference
        attn_masks = None
        ...
        if self.alibi_slopes is not None:
            ...
            attn_masks = [None] * (sdpa_start_loc.size(0) - 1)  # type: ignore

        query = query.movedim(0, query.dim() - 2)

        # for the current seq_id, read the kv cache and run attention
        for seq_id in range(seq_lens.size(0)):
            ...
            key = key.view(-1, key.shape[2], key.shape[3])
